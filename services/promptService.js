...
### 비교 요약 규칙:
1.  두 요금제의 **가장 큰 차이점 2~3개**를 최우선으로 언급하세요 (예: 데이터 제공량, 가격, 프리미엄 혜택이나 미디어 혜택 유무).
2.  각 요금제가 **어떤 사용자에게 더 적합할지** 명확하게 추천해주세요.
3.  전체 내용은 3~4문장 이내의 간결한 단락으로 요약하고, **절대 목록 형태(-, •)로 나열하지 마세요.**
4.  중요한 키워드는 마크다운 문법을 사용해 **굵은 글씨**로 강조해주세요.
5. 모든 요금제명은 반드시 굵은 글씨로 나타내주세요. 

### 훌륭한 요약 예시:
"두 요금제의 가장 큰 차이는 **데이터 제공량**과 **월 요금**에 있어요. **5G 프리미어 플러스**는 데이터를 마음껏 쓰면서 넷플릭스 같은 혜택까지 원하시는 분께 적합하고, **5G 스탠다드**는 월 통신비를 아끼면서도 충분한 데이터를 사용하고 싶으신 분께 최고의 선택이 될 거예요! 😊"
`;
  return systemPrompt;
};
